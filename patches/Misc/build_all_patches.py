...
pathToSpriggitEnv = os.getenv("SPRIGGIT_CLI_PATH")
pathToSkyrimEnv = os.getenv("SKYRIM_SE_PATH")
...
def scanForCandidate(pathToScan=Path(".")):
    for fsElement in pathToScan.iterdir():
        if (
            (fsElement.is_dir() and hasSpriggit(fsElement))
            or fsElement.name == "SKSE"
            or fsElement.name == "source"
            or fsElement.name == "textures"
            or fsElement.name == "meshes"
            or fsElement.suffix == ".esp"
            or fsElement.suffix == ".esl"
            or fsElement.suffix == ".esm"
        ):
            return True
...
if buildExists:
    print(
        f"{Fore.YELLOW}A 'build' directory already exists. Proceeding will remove it and it's contents{Style.RESET_ALL}"
    )

proceed = False
print(
    f"{Style.BRIGHT}Proceed with build?{Style.RESET_ALL} [{Fore.GREEN}Y{Style.RESET_ALL}/{Fore.RED}N{Style.RESET_ALL}]"
)
while not proceed:
    whatDoYouSay = input()
    if whatDoYouSay.lower() == "y":
        ...
        print(
            f"{Style.BRIGHT}Proceed?{Style.RESET_ALL} [{Fore.GREEN}Y{Style.RESET_ALL}/{Fore.RED}N{Style.RESET_ALL}]"
        )
...
for candidate in candidatePackages:
    ...
    if doSpriggit:
        if pathToSpriggitEnv is not None:
            pathToSpriggit = Path(pathToSpriggitEnv)
        else:
            print(
                f"{Back.RED + Fore.BLACK} SPRIGGIT CLI IS REQUIRED BUT ENVIRONMENT VARIABLE 'SPRIGGIT_CLI_PATH' IS EMPTY {Style.RESET_ALL}"
            )
            sys.exit(-1)
    ...
    for file in filesToCopy:
        if file.is_dir():
            ...
            if file.name.lower() == "scripts" or file.name.lower() == "source":
                doCompile = True
            shutil.copytree(
                file, "build/" + candidate + "/" + file.name, dirs_exist_ok=True
            )
        else:
    ...
    print(
        f"{Fore.BLUE + Back.GREEN + ' Build dir created. ' + str(numOfFiles) + Fore.BLACK} loose files and {Fore.BLUE + str(numOfDirectories) + Fore.BLACK} directories copied successfuly {Style.RESET_ALL}\n"
    )
    ...
    for plugin in spriggitTargets:
        ...
        try:
            subprocess.run(
                [pathToSpriggit, "deserialize", "-i", inputPath, "-o", outputPath],
                capture_output=True,
                shell=True,
                check=True,
            )
            ...
            sys.exit(-1)
        print(
            f" {Fore.GREEN + plugin.name + '.esp'} built successfully{Style.RESET_ALL}\n"
        )

    if len(spriggitTargets) > 0:
        print(
            f"{Back.GREEN + Fore.BLACK} All plugins for {Fore.BLUE + candidate + Fore.BLACK} built successfully {Style.RESET_ALL}\n"
        )
    else:
    ...
    if doCompile:
        if pathToSkyrimEnv:
            pathToSkyrim = Path(pathToSkyrimEnv)
        else:
            f"{Back.RED + Fore.BLACK} SCRIPT COMPILATION REQUIRED BUT ENVIRONMENT VARIABLE 'SKYRIM_SE_PATH' IS EMPTY {Style.RESET_ALL}"
            sys.exit(-1)

    compilerPath = pathToSkyrim / "Papyrus Compiler/PapyrusCompiler.exe"
    ...
    for lv1 in topLevelDir.iterdir():
        if lv1.name.lower() == "source":
            ...
            if compilerSource.exists():
                break
    if compilerSource != Path(".") and compilerSource.exists():
        papyrusIncludes = []
        ...
        for file in compilerSource.iterdir():
            if file.is_file() and file.suffix == ".psc":
                ...
                try:
                    ...
                    print(
                        f" {Fore.GREEN + file.name} compiled successfully{Style.RESET_ALL}"
                    )
                    numOfScripts = numOfScripts + 1
                except subprocess.CalledProcessError as e:
                    print(
                        f"{Back.RED + Fore.BLACK} FAILED TO COMPILE SCRIPTS {Style.RESET_ALL}"
                    )
                    print(e.cmd)
    ...
    shutil.make_archive(f"./build/{candidate}", "zip", Path(f"./build/{candidate}"))
    print(
        f"{Back.GREEN} {Fore.BLUE + candidate + '.zip' + Fore.BLACK} archive created successfully. {Style.RESET_ALL}\n"
    )
    shutil.rmtree(Path(f"./build/{candidate}/"))

print(f"\n{Back.GREEN}                                     {Style.RESET_ALL}")
print(
    f"{Back.GREEN + Fore.BLACK}          Build Successful           {Style.RESET_ALL}"
)
